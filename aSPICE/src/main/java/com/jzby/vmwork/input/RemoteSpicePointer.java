...
import com.jzby.vmwork.RfbConnectable;
...
public class RemoteSpicePointer extends RemotePointer {
    ...
    public class MouseScrollRunnable implements Runnable {
        ...
        @Override
        public void run() {
            Log.d("MouseScrollRunnable", "[jcktest] run()");
    ...
    public boolean handleHardwareButtons(int keyCode, KeyEvent evt, int combinedMetastate) {
        ...
        Log.d("MouseScrollRunnable", "[jcktest] handleHardwareButtons----start");
        Log.d("MouseScrollRunnable", "[jcktest] down:"+down+"pointerMask:"+pointerMask);
        ...
        Log.d("MouseScrollRunnable", "[jcktest] mouseChange:"+mouseChange+"keyCode:"+keyCode);
        if (shouldBeRightClick(evt))
        {
            ...
            Log.d("MouseScrollRunnable", "[jcktest] shouldBeRightClick----combinedMetastate:"+combinedMetastate);
            return true;
        }
        else if (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN || keyCode == KeyEvent.KEYCODE_VOLUME_UP)
        {
            ...
            rfb.writePointerEvent(getX(), getY(), combinedMetastate, pointerMask);
            Log.d("MouseScrollRunnable", "[jcktest] KEYCODE_VOLUME_DOWN----combinedMetastate:"+combinedMetastate);
            return true;
        }
        Log.d("MouseScrollRunnable", "[jcktest] handleHardwareButtons----------------end\r\n");
        return false;
    }
     ...
     *
     * @param action         motion event; x and y must already have been converted from screen coordinates
     *                       to remote frame buffer coordinates.
    ...
    public boolean processPointerEvent(int x, int y, int action, int modifiers, boolean mouseIsDown, boolean useRightButton,
        ...
        Log.d(TAG,"jcktest processPointerEvent x="+x+",y="+y);
        Log.d(TAG,"jcktest processPointerEvent modifiers="+modifiers+",mouseIsDown="+mouseIsDown+",action="+action);
        Log.d(TAG,"jcktest processPointerEvent useMiddleButton="+useMiddleButton+",useRightButton="+useRightButton+",useScrollButton="+useScrollButton+",direction"+direction);
        if (rfb != null && rfb.isInNormalProtocol())
        {
            if (useRightButton)
            {
                Log.d(TAG,"jcktest Mouse button right");
                pointerMask = SPICE_MOUSE_BUTTON_RIGHT;
            }
            else if (useMiddleButton)
            {
                Log.d(TAG,"jcktest Mouse button middle");
                pointerMask = SPICE_MOUSE_BUTTON_MIDDLE;
            }
            else if (action == MotionEvent.ACTION_DOWN)
            {
                Log.d(TAG,"jcktest Mouse button left");
            ...
            else if (useScrollButton)
            {
                 switch(direction)
                 {
                 	case 0:
			        {
				        Log.d(TAG,"jcktest Scrolling up");
                    	pointerMask = SPICE_MOUSE_BUTTON_UP;
			        }
			        break;
			        case 1:
			        {
				        Log.d(TAG,"jcktest Scrolling down");
            ...
            {
                Log.d(TAG,"jcktest Mouse moving");
            ...
            else
            {
                ...
                Log.d(TAG,"jcktest Setting previous mouse prevPointerMask:"+prevPointerMask);
            }
            ...
            Log.d(TAG,"jcktest Mouse prevPointerMask:"+prevPointerMask+"pointerMask:"+pointerMask);
            if (pointerMask != SPICE_MOUSE_BUTTON_MOVE)
            {
                ...
                {
                    int needPrevPointMask =  (prevPointerMask & ~PTRFLAGS_DOWN);
                    Log.d(TAG,"jcktest Mouse pointer up prevPointerMask prevpointer :"+needPrevPointMask);
                  // rfb.writePointerEvent(mouseX, mouseY, modifiers | vncCanvas.getKeyboard().getMetaState(),needPrevPointMask);
                }
                prevPointerMask = pointerMask;
                Log.d(TAG,"jcktest Mouse set prevPointerMask:"+prevPointerMask);
            }

            if (mouseIsDown)
            {
                Log.d(TAG,"jcktest Mouse pointer is down");
                pointerMask = pointerMask | PTRFLAGS_DOWN;
            }
            else
            {
                Log.d(TAG,"jcktest Mouse pointer is up");
                prevPointerMask = 0;
            }
	        ...
	        //jck add for up and left button not up
            if(direction == 88)
            {
                pointerMask |= SPICE_MOUSE_BUTTON_LEFT;
            }
            Log.d(TAG,"jcktest Mouse prevPointerMask :"+prevPointerMask+"pointerMask:"+pointerMask+"----end");
