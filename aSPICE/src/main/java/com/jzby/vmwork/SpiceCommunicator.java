...
public class SpiceCommunicator implements RfbConnectable {
    ...
    public native void SpiceKeyEvent(boolean keyDown, int virtualKeyCode,int meta_status);

    public native void UpdateBitmap(Bitmap bitmap, int x, int y, int w, int h);

    public native void UpdateCursor(Bitmap bitmap, int x, int y, int w, int h);

    public native void SpiceRequestResolution(int x, int y);

    public native void StartSessionFromVvFile(String vvFileName, boolean sound);
    ...
    public void setUIEventListener(UIEventListener ui) {
        //Log.e("yxlei", "setUIEventListener");
        Log.i(TAG,"=====setUIEventListener=====");
        uiEventListener = ui;
    }
    ...
    class SpiceThread extends Thread {
        ...
        public void run() {
            int result = SpiceClientConnect(ip, port, tport, password, cf, cs, sound);
            android.util.Log.e(TAG, "SpiceClientConnect returned=" + result);
    ...
    public void sendMouseEvent(int x, int y, int metaState, int pointerMask) {
        Log.d(TAG, "[jcktest]	sendMouseEvent x=" + x + ",y=" + y+"time:"+System.currentTimeMillis());
       // Log.i(TAG,"[jcktest]====xpz=====MouseEvent======="+System.currentTimeMillis());
        SpiceButtonEvent(x, y, metaState, pointerMask);
    }

    public void sendKeyEvent(boolean keyDown, int scanCode) {
        Log.d(TAG, "[jcktest]	*********************sendKeyEvent: " + scanCode + ". Is it down: " + keyDown);
        SpiceKeyEvent(keyDown, scanCode,1);
    }
    
    public void sendKeyEventWithMetastatus(boolean keyDown, int scanCode,int meta_status)
    {
    	  Log.d(TAG, "[jcktest]	*********sendKeyEventWithMetastatus: " + scanCode + ". Is it down: " + keyDown);
	  SpiceKeyEvent(keyDown, scanCode,meta_status);
    }
    /* Callbacks from jni */
    ...
    private static boolean OnAuthenticate(int inst, StringBuilder username, StringBuilder domain, StringBuilder password) {
        Log.i(TAG,"=====OnAuthenticate=====");
        if (uiEventListener != null)
            return uiEventListener.OnAuthenticate(username, domain, password);
        return false;
    }

    private static void OnCursorViewChanged(int type, int width, int height,int hot_x,int hot_y)
    {
        Log.i(TAG,"=====OnCursorViewChanged=====");
        if(uiEventListener != null)
        {
            uiEventListener.OnCursorViewChanged(type,width,height,hot_x,hot_y);
        }
    }

    private static void OnGraphicsUpdate(int inst, int x, int y, int width, int height) {
        Log.i(TAG,"=====OnGraphicsUpdate=====");
        if (uiEventListener != null)
    ...
    private static void OnGraphicsResize(int inst, int width, int height, int bpp) {
        android.util.Log.e(TAG, "onGraphicsResize, width: " + width + " height: " + height);
        if (uiEventListener != null)
    ...
    public void writePointerEvent(int x, int y, int metaState, int pointerMask) {
        Log.d(TAG, "[jcktest]	writePointerEvent x =" + x + ",y=" + y);
    ...
    private void sendModifierKeys(boolean keyDown, int key) {
        // Avoid sending meta keys down multiple times.
         Log.d(TAG, "[jcktest]sendModifierKeys---keyDown:"+keyDown+"         key:"+key);
        if ((metaState & RemoteKeyboard.CTRL_MASK) != 0) {
            if (!keyDown || key != RemoteKeyboard.SCAN_LEFTCTRL && (remoteMetaState & RemoteKeyboard.CTRL_MASK) == 0) {
                android.util.Log.v(TAG, "[jcktest]	Sending LCTRL: " + RemoteKeyboard.SCAN_LEFTCTRL + " down: " + keyDown);
        ...
        if ((metaState & RemoteKeyboard.RCTRL_MASK) != 0) {
            if (!keyDown || key != RemoteKeyboard.SCAN_RIGHTCTRL && (remoteMetaState & RemoteKeyboard.RCTRL_MASK) == 0) {
                android.util.Log.v(TAG, "[jcktest]	Sending RCTRL: " + RemoteKeyboard.SCAN_RIGHTCTRL + " down: " + keyDown);
        ...
        if ((metaState & RemoteKeyboard.ALT_MASK) != 0) {
            if (!keyDown || key != RemoteKeyboard.SCAN_LEFTALT && (remoteMetaState & RemoteKeyboard.ALT_MASK) == 0) {
                android.util.Log.v(TAG, "[jcktest]	Sending LALT: " + RemoteKeyboard.SCAN_LEFTALT + " down: " + keyDown);
        ...
        if ((metaState & RemoteKeyboard.RALT_MASK) != 0) {
            if (!keyDown || key != RemoteKeyboard.SCAN_RIGHTALT && (remoteMetaState & RemoteKeyboard.RALT_MASK) == 0) {
                android.util.Log.v(TAG, "[jcktest]	Sending RALT: " + RemoteKeyboard.SCAN_RIGHTALT + " down: " + keyDown);
        ...
        if ((metaState & RemoteKeyboard.SUPER_MASK) != 0) {
            if (!keyDown || key != RemoteKeyboard.SCAN_LEFTSUPER && (remoteMetaState & RemoteKeyboard.SUPER_MASK) == 0) {
                android.util.Log.v(TAG, "[jcktest]	Sending SUPER: " + RemoteKeyboard.SCAN_LEFTSUPER + " down: " + keyDown);
        ...
        if ((metaState & RemoteKeyboard.RSUPER_MASK) != 0) {
            if (!keyDown || key != RemoteKeyboard.SCAN_RIGHTSUPER && (remoteMetaState & RemoteKeyboard.RSUPER_MASK) == 0) {
                android.util.Log.v(TAG, "[jcktest]	Sending RSUPER: " + RemoteKeyboard.SCAN_RIGHTSUPER + " down: " + keyDown);
        ...
        if ((metaState & RemoteKeyboard.SHIFT_MASK) != 0) {
            if (!keyDown || key != RemoteKeyboard.SCAN_LEFTSHIFT && (remoteMetaState & RemoteKeyboard.SHIFT_MASK) == 0) {
                android.util.Log.v(TAG, "[jcktest]	Sending SHIFT: " + RemoteKeyboard.SCAN_LEFTSHIFT + " down: " + keyDown);
        ...
        if ((metaState & RemoteKeyboard.RSHIFT_MASK) != 0) {
            if (!keyDown || key != RemoteKeyboard.SCAN_RIGHTSHIFT && (remoteMetaState & RemoteKeyboard.RSHIFT_MASK) == 0) {
                android.util.Log.v(TAG, "[jcktest]	Sending RSHIFT: " + RemoteKeyboard.SCAN_RIGHTSHIFT + " down: " + keyDown);
        ...
        if (keyDown) {
		 ...
		 Log.d(TAG, "[jcktest]4444444444444444444444444---1remoteMetaState:"+remoteMetaState);
    }

    @Override
    public void writeKeyEvent(int key, int metaState, boolean keyDown) {
    Log.d(TAG, "[jcktest]	writeKeyEvent key:" + key + "metaState=" +metaState+"keyDown:"+keyDown);
       // if (keyDown) {
          //  this.metaState = metaState;
       ...
       sendKeyEventWithMetastatus(keyDown,key,metaState);
	   this.metaState =0;
    ...
    public void requestResolution(int x, int y) {
        Log.d(TAG, "requestResolution x=" + x + ",y=" + y);
